...
const quotes = [
  {
    ...
    grandTotal: 1000,
    turnaroundDays: 1,
    lineItems: [{ id: 'l1', layer: 'print', computedTotal: 1000, priceSnapshot: { name: 'Duplex' } }],
  },
  {
    id: 'q2',
...
describe('JobLogView', () => {
  it('renders quote list and filters by query', () => {
    render(<JobLogView loading={false} onDuplicateQuote={vi.fn()} onExportCsv={vi.fn()} quotes={quotes} />)
  ...
  it('duplicates quote to new draft', () => {
    ...
    render(<JobLogView loading={false} onDuplicateQuote={onDuplicateQuote} onExportCsv={vi.fn()} quotes={quotes} />)

    fireEvent.click(screen.getByRole('button', { name: 'Duplicate JOB-001' }))

    expect(onDuplicateQuote).toHaveBeenCalledWith(expect.objectContaining({ sourceQuoteId: 'q1' }))
  })

  it('exports visible quotes as CSV', () => {
    const onExportCsv = vi.fn()
    render(<JobLogView loading={false} onDuplicateQuote={vi.fn()} onExportCsv={onExportCsv} quotes={quotes} />)
