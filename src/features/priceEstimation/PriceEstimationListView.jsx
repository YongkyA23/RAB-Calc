...
export function PriceEstimationListView({ estimates, loading, onCreateNew, onDuplicateEstimate, onEditDraft, onExportCsv, onDeleteEstimate }) {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1.4fr_0.7fr]">
                ...
                <tr key={estimate.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <button
                        aria-label={`View ${estimateLabel(estimate)}`}
                      ...
                      {normalizeEstimateStatus(estimate) === 'draft' ? (
                        <>
                          <button
                            aria-label={`Edit Draft ${estimateLabel(estimate)}`}
                            className="rounded-lg border border-amber-200 px-3 py-1.5 text-xs font-semibold text-amber-700 hover:bg-amber-50"
                            onClick={() => onEditDraft(estimate)}
                            type="button"
                          >
                            Edit
                          </button>
                          <button
                            aria-label={`Delete Draft ${estimateLabel(estimate)}`}
                            className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50"
                            onClick={() => setDraftToDelete(estimate)}
                            type="button"
                          >
                            Delete
                          </button>
                    ...
                    {draftToDelete === estimate && onDeleteEstimate ? (
                      <div className="mt-2 rounded-lg border border-red-200 bg-red-50 p-3">
                        <p className="text-xs font-semibold text-red-800">Confirm deletion of draft "{estimateLabel(estimate)}"</p>
      ...
      <aside className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="text-base font-bold text-slate-950">Estimate detail</h3>
        {selectedEstimate ? (
          <div className="mt-4 space-y-4 text-sm text-slate-600">
            <div>
              <p className="font-semibold text-slate-900">{estimateLabel(selectedEstimate)}</p>
              <p>{selectedEstimate.client || '-'} · {selectedEstimate.project || '-'}</p>
              <p>{formatIdr(selectedEstimate.grandTotal)} · {selectedEstimate.turnaroundDays ?? 0} days</p>
            </div>
          ...
          <p className="mt-4 text-sm text-slate-600">Select an estimate to inspect saved line-item snapshots.</p>
        )}
      </aside>
    </div>
  )
