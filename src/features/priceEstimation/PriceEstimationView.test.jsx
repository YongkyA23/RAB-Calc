...
describe('PriceEstimationView', () => {
  it('confirms before deleting draft estimate', () => {
    ...
    render(<PriceEstimationListView estimates={estimates} loading={false} onCreateNew={vi.fn()} onDuplicateEstimate={vi.fn()} onEditDraft={vi.fn()} onExportCsv={vi.fn()} onDeleteEstimate={onDeleteEstimate} />)
  ...
  it('confirms before deleting created estimate', () => {
    const onDeleteEstimate = vi.fn()
    render(<PriceEstimationListView estimates={[estimates[1]]} loading={false} onCreateNew={vi.fn()} onDuplicateEstimate={vi.fn()} onEditDraft={vi.fn()} onExportCsv={vi.fn()} onDeleteEstimate={onDeleteEstimate} />)
